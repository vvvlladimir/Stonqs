...
- A reading is shown while it happens: `AiEvent::ToolRunning` / `ToolFinished` / `Searching` drive
  ...
  its count.
- **The panel says it is one.** A standing line under the composer (`.ai-note`), and its twin
  under the summary tile's text (`.brief__note`): generated by AI, can be wrong, not investment
  advice. Not dismissible and not a tooltip — it is shown wherever an answer is (ADR-0078).
